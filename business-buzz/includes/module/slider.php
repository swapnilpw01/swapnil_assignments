...
if ( ! function_exists( 'business_buzz_add_featured_slider' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function business_buzz_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'business_buzz_filter_slider_status', true );
		...
		$slider_details = apply_filters( 'business_buzz_filter_slider_details', $slider_details );

		if ( empty( $slider_details ) ) {
			return;
		}

		business_buzz_render_featured_slider( $slider_details );

	}

endif;

add_action( 'business_buzz_action_before_content', 'business_buzz_add_featured_slider', 5 );

if ( ! function_exists( 'business_buzz_check_slider_status' ) ) :
	 ...
	 */
	function business_buzz_check_slider_status( $input ) {

		$input = false;

		// Slider status.
		$featured_slider_status = business_buzz_get_option( 'featured_slider_status' );
...
add_filter( 'business_buzz_filter_slider_status', 'business_buzz_check_slider_status' );

if ( ! function_exists( 'business_buzz_get_slider_details' ) ) :
	 ...
	 */
	function business_buzz_get_slider_details( $input ) {

		$featured_slider_type           = business_buzz_get_option( 'featured_slider_type' );
		$featured_slider_number         = business_buzz_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = business_buzz_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = business_buzz_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							$slides[ $cnt ]['excerpt'] = business_buzz_get_the_excerpt( apply_filters( 'business_buzz_filter_slider_caption_length', 30 ), $post );
...
add_filter( 'business_buzz_filter_slider_details', 'business_buzz_get_slider_details' );

if ( ! function_exists( 'business_buzz_render_featured_slider' ) ) :
	 ...
	 */
	function business_buzz_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}

		$featured_slider_transition_effect   = business_buzz_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = business_buzz_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = business_buzz_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = business_buzz_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = business_buzz_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_transition_duration = business_buzz_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = business_buzz_get_option( 'featured_slider_transition_delay' );
