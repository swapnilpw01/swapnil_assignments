...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ) {
      	...
      	var BusinessBuzzCustomThemeImage = wp.media.controller.Library.extend({
      		defaults : _.defaults({
        ...
        business_buzz_file_frame = wp.media.frames.business_buzz_file_frame = wp.media({
        	button: {
        		text: jQuery( this ).data( 'uploader_button_text' )
        	},
        	state : 'custom-theme-insert-image',
        	states : [
        	new BusinessBuzzCustomThemeImage()
        	],
        	multiple: false
        });

        // When an image is selected, run a callback.
        business_buzz_file_frame.on( 'select', function() {

        	var state = business_buzz_file_frame.state('custom-theme-insert-image');
